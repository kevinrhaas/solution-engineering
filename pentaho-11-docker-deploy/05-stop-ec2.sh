...
# Stop an existing EC2 instance from runtime state
...
if [ "${CURRENT_STATE}" = "stopped" ]; then
    ...
    aws_cmd ${AWS_PROFILE} ec2 stop-instances \
        --region ${AWS_REGION} \
        --instance-ids ${INSTANCE_ID} >/dev/null

    echo "⏳ Waiting for stopped state..."
    aws_cmd ${AWS_PROFILE} ec2 wait instance-stopped \
        --region ${AWS_REGION} \
        --instance-ids ${INSTANCE_ID}
fi
...
PRIVATE_IP=$(echo "${INSTANCE_INFO}" | awk '{print $1}')
FINAL_STATE=$(echo "${INSTANCE_INFO}" | awk '{print $2}')

if [ "${PRIVATE_IP}" = "None" ]; then PRIVATE_IP=""; fi
if [ -z "${FINAL_STATE}" ] || [ "${FINAL_STATE}" = "None" ]; then FINAL_STATE="stopped"; fi

upsert_state_var "${RUNTIME_STATE}" "INSTANCE_STATE" "${FINAL_STATE}"
upsert_state_var "${RUNTIME_STATE}" "PUBLIC_IP" ""
